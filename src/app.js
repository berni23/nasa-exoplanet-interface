jQuery(function () {
  ...
  function initialize() {
    ...
    requestAPI("picture").then(function (res) {
      ...
      Object.keys(pictures).forEach((key) =>
        populatePicture(key, pictures[key])
      );
    });
  }

  function populatePicture(key, picture) {
    var parent = "picture-" + key;

    console.log(key, parent);
    console.log(picture);
    $(`#${parent} .picture-title`).text(picture.title);
    $(`#${parent} .picture-date`).text(picture.date);
    var explanationContent = $(`<p>${picture.explanation}</p>`);
    $(`#${parent} .explanation-wrapper`).empty();
    $(`#${parent} .explanation-wrapper`).append(explanationContent);
    $(`#${parent} .today-image`).attr(
      "src",
      picture.hdurl ? picture.hdurl : picture.url
    );
    $(`li[data-content=${parent}]>span`).text(picture.title);
  }

  function queryApi() {
    wait.removeClass("hidden");
    requestAPI("data", "exoplanets").then(() =>
  ...
  menuItems.on("click", function (event) {
    ...
    if (li.hasClass("my-content") && !li.hasClass("content-active")) {
      ...
      plotTitle.text(li.attr("data-title"));
    }
  });
        ...
        if (id in configObject) showCanvas(id);
          ...
          switch (id) {
            ...
            case "chart-1":
              ...
              break;
            case "chart-1-2":
              plotDisc("chart-1-2", "pl_facility").then((config) =>
                plotChart(config)
              );
              break;
            case "chart-1-3":
  ...
  function plotDisc(id, column = "pl_discmethod") {
    if (!(id in configObject)) {
      return getColumns(
        ["pl_orbsmax", "pl_radj", "pl_hostname", column],
        ["pl_orbsmax", "pl_radj"]
      ).then(function (data) {
